import { last, pullAt, omit, cloneDeep } from 'es-toolkit'
import {
    assign,
    times,
    get,
...
export default class TreeManage<T extends Record<string, any>, K extends string> {
    ...
    public init(tree: T[], config?: Partial<configType<T>>) {
        ...
        const run = (tree: T[], parentNode: T | undefined, parentPath: string) => {
            const nodes: T[] = []
    ...
    public merge(from: T[], rootKey: K) {
        ...
        const merge = (from: T[], parentNode: T | undefined) => {
            if (!parentNode) {
                return
            }
